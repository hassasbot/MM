...
class MarketDataHub:
    ...
    async def get_klines(self, symbol: str, bar: str, limit=200):
        okx_bar = bar.replace("m", "m").replace("H", "H")  # already correct
        data = await self.fetch_with_retry("/api/v5/market/candles",
                                           {"instId": symbol, "bar": okx_bar, "limit": limit})
        if not data: return []
        candles = []
    ...
    async def get_order_book(self, symbol):
        data = await self.fetch_with_retry("/api/v5/market/books",
                                           {"instId": symbol, "sz": 50})
    ...
    async def get_agg_trades(self, symbol):
        data = await self.fetch_with_retry("/api/v5/market/trades",
                                           {"instId": symbol, "limit": 500})
        if data:
            for t in reversed(data[-150:]):
                side = t.get("side", "")
                is_buyer_maker = (side == "sell")  # taker sold -> maker bought
                trade = {
                    "price": float(t["px"]),
                    "qty": float(t["sz"]),
    ...
    # Latency recording for REST (timestamp from response)
...
class OrderflowDeltaEngine:
    ...
    def get_delta_score(self, symbol: str, direction: str) -> float:
        ...
        if delta > 0 and direction == "BUY":
            return 10.0
        elif delta < 0 and direction == "SELL":
            return 10.0
        elif delta > 0 and direction == "SELL":
            return -5.0
        elif delta < 0 and direction == "BUY":
            return -5.0
        return 0.0


# ---------- MARKET CONTEXT ENGINE (2026 Calendar) ----------
class MarketContextEngine:
    def __init__(self, dm: MarketDataHub):
        ...
        # 2026 Economic Calendar (UTC)
        self.economic_events = [
            # FOMC
            ("2026-01-28", "19:00", "FOMC", 120),
            ("2026-03-18", "18:00", "FOMC", 120),
            ("2026-05-06", "18:00", "FOMC", 120),
            ...
            # CPI
            ("2026-01-14", "13:30", "CPI", 90),
            ("2026-02-11", "13:30", "CPI", 90),
            ("2026-03-11", "12:30", "CPI", 90),
            ("2026-04-15", "12:30", "CPI", 90),
            ("2026-05-13", "12:30", "CPI", 90),
            ...
            # NFP
            ("2026-01-09", "13:30", "NFP", 90),
            ("2026-02-06", "13:30", "NFP", 90),
            ("2026-03-06", "13:30", "NFP", 90),
            ("2026-04-03", "12:30", "NFP", 90),
            ("2026-05-01", "12:30", "NFP", 90),
            ...
            # GDP
            ("2026-01-29", "13:30", "GDP", 90),
            ("2026-04-29", "12:30", "GDP", 90),
            ("2026-07-30", "12:30", "GDP", 90),
            ("2026-10-29", "12:30", "GDP", 90),
            # PPI
            ("2026-01-13", "13:30", "PPI", 60),
            ("2026-02-10", "13:30", "PPI", 60),
            ("2026-03-10", "12:30", "PPI", 60),
            ("2026-04-14", "12:30", "PPI", 60),
            ("2026-05-12", "12:30", "PPI", 60),
            ("2026-06-09", "12:30", "PPI", 60),
            ("2026-07-14", "12:30", "PPI", 60),
            ("2026-08-11", "12:30", "PPI", 60),
            ("2026-09-15", "12:30", "PPI", 60),
            ("2026-10-13", "12:30", "PPI", 60),
            ("2026-11-17", "13:30", "PPI", 60),
            ("2026-12-15", "13:30", "PPI", 60),
            # Retail Sales
            ("2026-01-15", "13:30", "Retail Sales", 60),
            ("2026-02-13", "13:30", "Retail Sales", 60),
            ("2026-03-17", "12:30", "Retail Sales", 60),
            ("2026-04-14", "12:30", "Retail Sales", 60),
            ("2026-05-14", "12:30", "Retail Sales", 60),
...
class LossPatternFilter:
    def __init__(self):
        self.patterns: Dict[tuple, Dict[str, int]] = {}  # (regime, direction, rsi_bucket, session, temp_bucket) -> {"loss": n, "win": m}

    def _bucket_rsi(self, rsi: float) -> str:
        if rsi < 35: return "low"
        elif rsi < 65: return "mid"
        else: return "high"

    def _bucket_temp(self, temp: float) -> str:
        if temp < 40: return "cold"
        elif temp < 70: return "normal"
        else: return "hot"

    def record(self, regime: str, direction: str, rsi: float, session: str, temperature: float, pnl: float):
        key = (regime, direction, self._bucket_rsi(rsi), session, self._bucket_temp(temperature))
        if key not in self.patterns:
            self.patterns[key] = {"loss": 0, "win": 0}
        if pnl <= 0:
            self.patterns[key]["loss"] += 1
        else:
            self.patterns[key]["win"] += 1

    def should_reject(self, regime: str, direction: str, rsi: float, session: str, temperature: float) -> bool:
        key = (regime, direction, self._bucket_rsi(rsi), session, self._bucket_temp(temperature))
        if key in self.patterns:
            stats = self.patterns[key]
            total = stats["loss"] + stats["win"]
            if total >= 3 and stats["loss"] / total > 0.6:
                return True
        return False
...
# ---------- SIGNAL GENERATOR (without ML, with LossPattern) ----------
class SignalGenerator:
    ...
    async def generate(self, symbol: str) -> Optional[Signal]:
        ...
        # Apply context score to confidence
    ...
    def report_result(self, pnl: float, regime: str, direction: str, rsi: float, session: str, temperature: float):
        if pnl <= 0:
...
# ---------- MARKET MENTOR ENGINE (unchanged) ----------
# [تمام متدها همان‌طور که در نسخه قبلی بودند، فقط آدرس API جدید استفاده می‌شود.]
class MarketMentorEngine:
    def __init__(self, dm: MarketDataHub):
        self.dm = dm
        self.m = MathUtils()
    # (تمامی ۱۷ متد بدون تغییر، فقط نماد SOL-USDT-SWAP استفاده می‌شود)
    ...
    async def swing_analysis(self, symbol: str) -> str:
        ...
        closes_1h = [c.close for c in candles_1h]
        highs_1h = [c.high for c in candles_1h]; lows_1h = [c.low for c in candles_1h]
        ...
        while True:
            ...
            for symbol in CONFIG["SYMBOLS"]:
                ...
                for tid in closed:
                    ...
                    if t_closed:
                        if self.signal_gen:
                            # Provide needed context for loss pattern
                            rsi = MathUtils.rsi([c.close for c in c5m]) if len(c5m) >= 14 else 50
                            session = self.signal_gen.context_engine.get_session()
                            temp = self.signal_gen.market_temp.get_temperature(symbol) if hasattr(self.signal_gen, 'market_temp') else 50
...
if TELEGRAM_AVAILABLE:
    class TelegramBot:
        ...
        async def startup(self): await self._send("🚀 OKX SOL Scalper started")
...
class RealTimeMarketEngine:
    def __init__(self, dm: MarketDataHub, pipeline):
        ...
        self.tfs = CONFIG["TIMEFRAMES"]["scalp"] + CONFIG["TIMEFRAMES"]["midterm"]  # ['1m','5m','15m','1H']
    ...
    async def _poll_all(self):
        ...
        for tf in self.tfs:
            try:
                ...
                if candles:
                    self.dm.candle_buffers[sym][tf].append(candles[0])
            except Exception as e:
        ...
        try:
            ...
            logger.warning(f"Failed to fetch orderbook/trades: {e}")
    ...
    async def start(self):
        ...
        for symbol in CONFIG["SYMBOLS"]:
            for tf in CONFIG["TIMEFRAMES"]["scalp"] + CONFIG["TIMEFRAMES"]["midterm"]:
                ...
                if candles:
                    self.dm.candle_buffers[symbol][tf].extend(candles)
