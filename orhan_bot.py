#!/usr/bin/env python3
# ============================================
# ultimate_bybit_bot_RENDER_COMPLETE.py
# ALL original engines + REST-only + Web server
# NO xgboost, NO WebSocket
# Fixed Binance connection (DNS bypass)
# ============================================
...
# ---------- MARKET DATA HUB (REST only, with DNS bypass) ----------
class MarketDataHub:
    def __init__(self):
        self.base = CONFIG["BINANCE_PUBLIC_API"]
        self.sess = None
        self.ssl = ssl.create_default_context()
        self.rate_limiter = RateLimiter()
        self.symbols = CONFIG["SYMBOLS"]
        self.candle_buffers: Dict[str, Dict[str, Deque[Candle]]] = {
            sym: {tf: deque(maxlen=CONFIG["CANDLE_LIMITS"][tf]) for tf in CONFIG["TIMEFRAMES"]}
        ...
        # Pre-resolved Binance IPs (to bypass potential DNS blocks)
        self._binance_ips = ["54.230.49.10", "54.230.49.59", "54.230.49.62"]
    ...
    async def get_session(self):
        if self.sess is None or self.sess.closed:
            ...
            # Custom connector to force direct connection
    ...
    async def update_klines(self, symbol, interval):
        data = await self.fetch_with_retry("/api/v3/klines", {"symbol": symbol, "interval": interval, "limit": CONFIG["CANDLE_LIMITS"][interval]})
        if data:
            candles = []
            for k in data:
                try:
                    candles.append(Candle(
                        ...
                        volume=float(k[5]), symbol=symbol, timeframe=interval, trades=k[8]
                    ))
                except: pass
            if candles:
    ...
    async def update_all_klines(self, symbol):
        tasks = [self.update_klines(symbol, tf) for tf in CONFIG["TIMEFRAMES"]]
        await asyncio.gather(*tasks, return_exceptions=True)

    async def get_ticker_24hr(self, symbol):
        return await self.fetch_with_retry("/api/v3/ticker/24hr", {"symbol": symbol}) or {}

    async def get_order_book(self, symbol):
        data = await self.fetch_with_retry("/api/v3/depth", {"symbol": symbol, "limit": 50})
        if data:
    ...
    async def get_agg_trades(self, symbol):
        data = await self.fetch_with_retry("/api/v3/aggTrades", {"symbol": symbol, "limit": 500})
        if data:
    ...
    async def get_open_interest(self, symbol):
        data = await self.fetch_with_retry("/fapi/v1/openInterest", {"symbol": symbol})
    ...
    async def get_funding_rate(self, symbol):
        data = await self.fetch_with_retry("/fapi/v1/premiumIndex", {"symbol": symbol})
    ...
    async def get_current_price(self, symbol):
        data = await self.fetch_with_retry("/api/v3/ticker/price", {"symbol": symbol})
        if data and "price" in data: return float(data["price"])
        return None

    async def get_best_bid_ask(self, symbol):
        data = await self.fetch_with_retry("/api/v3/ticker/bookTicker", {"symbol": symbol})
        if data:
...
class SignalGenerator:
    ...
    async def generate(self, symbol: str) -> Optional[Signal]:
        ...
        if context_score < 30:
            logger.info(f"Market context too poor – no signal")
            return None
...
class MarketMentorEngine:
    ...
    async def full_analysis(self, symbol: str) -> str:
        ...
        return (f"📊 تحلیل SOLUSDT (قیمت: {price:.4f})\n"
    ...
    async def daily_suggestion(self, symbol: str) -> str:
        ...
        return (f"📅 پیشنهاد روز SOLUSDT (قیمت: {price:.4f})\n"
                f"• روند ۱h: {trend_1h}\n"
    ...
    async def key_levels(self, symbol: str) -> str:
        ...
        lines = [f"📊 سطوح کلیدی SOLUSDT (قیمت فعلی: {price:.4f})"]
        if sorted_res:
    ...
    async def pending_setup(self, symbol: str) -> str:
        ...
        header=f"🎯 پندینگ روزانه SOLUSDT ({price:.4f})\nروند: {trend}\n"
        return header + "\n\n".join(suggestions)
    ...
    async def pending_setup_scalp(self, symbol: str) -> str:
        ...
        return f"🎯 پندینگ اسکالپ SOLUSDT ({price:.4f})\n• روند ۵m: {trend}\n• ATR:{atr:.4f}\n\n" + "\n\n".join(suggestions)
        ...
        while True:
            ...
            for symbol in CONFIG["SYMBOLS"]:
                ...
                for tid in closed:
                    ...
                    if t_closed:
                        if self.signal_gen:
                            self.signal_gen.report_result(t_closed.pnl_pct)
...
if TELEGRAM_AVAILABLE:
    class TelegramBot:
        ...
        async def startup(self): await self._send("🚀 *ربات SOL روی Render فعال شد*\n✅ تمام موتورها فعال")

        async def signal(self, sig: Signal):
            em = "📈" if sig.direction == "BUY" else "📉"
            d = "خرید" if sig.direction == "BUY" else "فروش"
        ...
        async def _start_command(self, update, context):
            ...
            await update.message.reply_text("🤖 *ربات SOL روی Render*", reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
        ...
        async def _health_command(self, update, context):
            ...
            if not dm: return
            msg = (f"📊 *وضعیت ربات*\n"
                   f"• درخواست‌های API: {dm.request_count}\n"
                   f"• خطاها: {dm.error_count}\n"
                   f"• کندل‌های 5m: {len(dm.candle_buffers['SOLUSDT'].get('5m',[]))}")
        ...
        async def _trades_command(self, update, context):
            ...
            dm = self.mentor.dm if self.mentor else None
            price = await dm.get_current_price("SOLUSDT") if dm else None
            lines = []
        ...
        async def _key_levels_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.key_levels("SOLUSDT")
            await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        ...
        async def _full_report_command(self, update, context):
            if not self.mentor: return
            report = await self.mentor.full_report("SOLUSDT")
            if len(report) > 4000:
...
class UltimateBybitBot:
    ...
    async def rest_update_loop(self):
        """Update klines and market data via REST"""
    ...
    async def analysis_loop(self):
        """Run signal generation periodically"""
        while self.is_running:
            for symbol in CONFIG["SYMBOLS"]:
                try:
