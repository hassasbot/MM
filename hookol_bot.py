#!/usr/bin/env python3
# ============================================
# hassan_final_scalp_bot_v12_REST_ONLY.py
# V12 – REST Only (No WebSocket) for Render 24/7
# ============================================
...
logger = logging.getLogger('HassanBotV12')
...
CONFIG = {
    ...
    "CANDLE_LIMITS": {"1m": 150, "5m": 100, "15m": 80, "1h": 60},
    ...
    "DAILY_LOSS_CIRCUIT_BREAKER": 2.5, "VOLATILITY_CIRCUIT_BREAKER": 2.0,
    ...
    "REST_UPDATE_INTERVAL": 10,  # Update candles every 10 seconds
}
...
class MarketDataHub:
    ...
    async def ensure_session(self):
        if not self.sess or self.sess.closed:
            ...
            self.sess = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=20), connector=connector)
    ...
    async def spread(self, sym):
        ...
        if d:
            ...
            return {"bid":b,"ask":a,"spread_pct":(a-b)/b*100 if b else 0}
    ...
    async def volume_24h(self, sym):
        ...
        return float(d["quoteVolume"]) if d and "quoteVolume" in d else 0

    async def update_klines(self, sym, interval):
        candles = []
        d=await self.fetch("/api/v3/klines",{"symbol":sym,"interval":interval,"limit":CONFIG["CANDLE_LIMITS"][interval]})
        if d:
            for k in d:
                try:
        ...
        if candles:
            self.candles[sym][interval] = deque(candles, maxlen=CONFIG["CANDLE_LIMITS"][interval])

    async def update_all_klines(self, sym):
        tasks = [self.update_klines(sym, tf) for tf in CONFIG["TIMEFRAMES"]]
...
class MomentumBreakoutEngine:
    ...
    def analyze(candles):
        ...
        if avg_vol_recent<avg_vol_hist*1.3: return {"direction":"NEUTRAL","score":0,"strength":"بی‌نوسان"}
        if direction=="LONG" and sum(1 for c in recent if c.is_bullish)<2: return {"direction":"NEUTRAL","score":0,"strength":"بی‌نوسان"}
        if direction=="SHORT" and sum(1 for c in recent if not c.is_bullish)<2: return {"direction":"NEUTRAL","score":0,"strength":"بی‌نوسان"}
...
class MarketStructureEngine:
    ...
    def analyze(candles):
        ...
        if current<last_low*1.02 and current>last_low*0.98: return {"position":"کف ساختاری 🟢","bias":"LONG","score":80}
        if current>last_high*0.98 and current<last_high*1.02: return {"position":"سقف ساختاری 🔴","bias":"SHORT","score":80}
        if current>last_high*1.02: return {"position":"شکست صعودی 🚀","bias":"LONG","score":90}
        if current<last_low*0.98: return {"position":"شکست نزولی 💥","bias":"SHORT","score":90}
...
class CorrelationFilter:
    ...
    def check(self, sym, signal_direction):
        ...
        if corr>0.5 and signal_direction!=btc_direction: return False
        return True
...
class TrendFilterEngine:
    ...
    def analyze(self, sym):
        ...
        if not ema9 or not ema21: return {"trend":"NEUTRAL","allowed_long":True,"allowed_short":True,"reason":"EMA ناکافی"}
...
class SharpMoveDetector:
    ...
    def detect(self, sym):
        ...
        self.last_alert=now
        return {"detected":True,"direction":"LONG" if last.close>prev.close else "SHORT","price":last.close,"move_pct":round(move_pct,2)}
...
class UltimateSignalGenerator:
    ...
    async def generate(self, sym):
        ...
        if vol_24h<CONFIG["MIN_LIQUIDITY_USD"]: return None
        ...
        if sharp:
            ...
            sig=ScalpSignal(
                ...
                take_profit=[tp1,tp2,tp3],confidence=80,star_rating="⭐⭐⭐⭐",
                momentum_strength="انفجاری 🚀",whale_status="نامشخص",
                market_structure="حرکت شارپ",risk_reward=round(abs(tp3-entry)/abs(entry-sl),2) if entry!=sl else 2.0,
                reason_summary=f"Sharp Move {sharp['move_pct']}%",
                timestamp=now,risk_percent=CONFIG["RISK_PER_TRADE"],position_size=ps,signal_type="SHARP"
            )
        ...
        if lv == sv: return None
        init = "LONG" if lv > sv else "SHORT"

        delta_r=self.delta.analyze(sym, init)
        total_score=sum(r.get("score",0) for _,r in pre+[("delta",delta_r)] if r.get("direction")==init)
        confidence=min(95, total_score/6*0.85)

        if init=="LONG" and not trend_result["allowed_long"]: return None
        if init=="SHORT" and not trend_result["allowed_short"]: return None
        if trend_result["trend"]=="NEUTRAL" and mom_r.get("strength") not in ["قوی","انفجاری 🚀"]: return None
        ...
        if (atr/price)*100<CONFIG["MIN_ATR_PCT"]: return None
        ...
        sig=ScalpSignal(
            ...
            risk_reward=round(abs(tp3-price)/abs(price-sl),2) if price!=sl else 0,
        ...
        self.last_sig[sym]=now; self.daily_trades+=1
        logger.info(f"🎯 SIGNAL {sym} {init} Conf:{confidence:.0f}%")
        return sig
...
class TradeManager:
    ...
    def open(self, sig):
        if len(self.active)>=CONFIG["MAX_OPEN_TRADES"] or self.sym_count.get(sig.symbol,0)>=CONFIG["MAX_OPEN_PER_SYMBOL"]:
            return None
        t=ActiveTrade(signal_id=sig.id,symbol=sig.symbol,direction=sig.direction,
...
if TELE:
    class TelegramBot:
        ...
        async def startup(self):
            try: await self.bot.send_message(self.channel,"🚀 *ربات V12 REST فعال شد*\n✅ بدون WebSocket\n✅ 24/7 پایدار",parse_mode=ParseMode.MARKDOWN)
            except: pass

        async def _start(self, u, c):
            kb=[["⚡ تحلیل اسکالپ","📊 سلامت"],["📋 معاملات","📈 گزارش"]]
            await u.message.reply_text("🤖 *V12*",reply_markup=ReplyKeyboardMarkup(kb,resize_keyboard=True),parse_mode=ParseMode.MARKDOWN)
        ...
        async def _scalp(self, u, c):
            try:
                ...
                mom=MomentumBreakoutEngine.analyze(c5); ms=MarketStructureEngine.analyze(c5)
...
class HassanBot:
    ...
    async def init_data(self):
        ...
        for sym in CONFIG["SYMBOLS"]:
            await self.hub.update_all_klines(sym)
        logger.info("✅ Ready")

    async def rest_update_loop(self):
        """Update candles and market data periodically"""
        while True:
            for sym in CONFIG["SYMBOLS"]:
                try:
                    await self.hub.update_all_klines(sym)
                    await self.hub.orderbook(sym)
                    await self.hub.agg_trades(sym)
                except Exception as e:
                    logger.error(f"Update error {sym}: {e}")
        ...
        while True:
            for sym in CONFIG["SYMBOLS"]:
                if not locks[sym].locked():
                    asyncio.create_task(self._analyze(sym, locks[sym]))
            await asyncio.sleep(5)

    async def _analyze(self, sym, lock):
        async with lock:
            try:
    ...
    async def start(self):
        logger.info("🥇 V12 REST Starting – Stable 24/7")
        asyncio.create_task(run_web_server())
        await self.init_data()
        await self.tele.start_polling()
        await self.tele.startup()
        self.breaker.reset_if_new_day()
        await asyncio.gather(
            self.rest_update_loop(),
            self.analysis_loop(),
            self.tm.monitor(self.tele.trade_update)
        )
