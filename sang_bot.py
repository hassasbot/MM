...
# ============================================
# ultimate_okx_bot.py
# Institutional SOL Scalper – OKX Swap (SOL-USDT-SWAP)
# Deploy-ready for Render.com
# + Persistent LossPattern (JSON) + Online AI (XGBoost)
# + Reversal cooldown fix + Trade regime fix
# + Adjusted filters (higher cooldown, min confidence, volume filter)
# ============================================

import asyncio, json, time, ssl, logging, os, sys, random, pickle
...
file_handler = RotatingFileHandler('okx_bot.log', maxBytes=5*1024*1024, backupCount=3, encoding='utf-8')
file_handler.setFormatter(logging.Formatter('[%(asctime)s] %(levelname)s: %(message)s'))
logger.addHandler(file_handler)

# ---------- CONFIG (Adjusted filters) ----------
CONFIG = {
    ...
    "MIN_CONFIDENCE": 50,              # increased from 45
    ...
    "SIGNAL_COOLDOWN": 600,            # increased from 300 (10 minutes)
    "SYMBOL_COOLDOWN": 45,
    "HEALTH_CHECK_INTERVAL": 30,
    "SHARP_MOVE_PCT_THRESHOLD": 0.5,
    "POLL_INTERVAL_SECONDS": 10,
    "HEALTH_SERVER_PORT": 10000,
    "ML_MODEL_PATH": "ml_model_v2.pkl",
    "ML_MIN_SAMPLES": 30,
}
...
class Candle:
    ...
    def __post_init__(self):
        ...
        if self.high < self.low:
            ...
            logger.warning(f"Candle fixed: high<low for {self.symbol} {self.timeframe} at {self.timestamp}")
...
class MarketDataHub:
    def __init__(self):
        ...
        self.latency_buffer: Dict[str, Deque[float]] = {sym: deque(maxlen=300) for sym in self.symbols}
    ...
    async def get_session(self):
        if self.sess is None or self.sess.closed:
            ...
            self.sess = aiohttp.ClientSession(timeout=timeout)
    ...
    async def get_klines(self, symbol: str, bar: str, limit=200):
        data = await self.fetch_with_retry("/api/v5/market/candles",
        ...
        for k in reversed(data):
            try:
                ...
                candles.append(Candle(
                    ...
                    volume=vol, symbol=symbol, timeframe=bar, trades=0
                ))
            except: pass
        if bar == "1H" and candles:
            self.price_history[symbol].append(candles[-1].close)
        return candles

    async def get_ticker_24hr(self, symbol):
        data = await self.fetch_with_retry("/api/v5/market/ticker", {"instId": symbol})
        if data and isinstance(data, list) and len(data) > 0:
            t = data[0]
            return {"volume": float(t.get("vol24h", "0")), "lastPrice": float(t.get("last", "0"))}
        return {}

    async def get_order_book(self, symbol):
        data = await self.fetch_with_retry("/api/v5/market/books", {"instId": symbol, "sz": 50})
    ...
    async def get_agg_trades(self, symbol):
        data = await self.fetch_with_retry("/api/v5/market/trades", {"instId": symbol, "limit": 500})
        if data:
            for t in reversed(data[-150:]):
                side = t.get("side", "")
    ...
    async def get_open_interest(self, symbol):
        data = await self.fetch_with_retry("/api/v5/public/open-interest", {"instId": symbol})
        if data and len(data) > 0:
    ...
    async def get_funding_rate(self, symbol):
        data = await self.fetch_with_retry("/api/v5/public/funding-rate", {"instId": symbol})
        if data and len(data) > 0:
            fr = float(data[0].get("fundingRate", 0))
            self.funding_rate[symbol] = fr
            return fr
        return None
    ...
    def update_latency(self, event_time_ms: int):
        sym = self.symbols[0]
        now_ms = time.time() * 1000
        latency = now_ms - event_time_ms
        self.latency_buffer[sym].append(latency)

    def get_avg_latency(self) -> float:
        sym = self.symbols[0]
        if len(self.latency_buffer[sym]) < 2:
            return 0.0
        return MathUtils.mean(list(self.latency_buffer[sym]))


# ---------- ENGINES ----------
# (WhaleIntelligenceEngine, OrderBookEngine, MarketStructureEngine, CandlePatternDetector,
#  LiquidityEngine, MarketRegimeDetector, MultiTimeframeAnalyzer – identical to previous full version)
...
# ---------- CVD, VOLUME PROFILE, OI, SHARP MOVE ----------
class CumulativeVolumeDelta:
    def __init__(self, dm: MarketDataHub): self.dm = dm; self.cvd_history: Dict[str, Deque[Tuple[datetime, float]]] = {}
    def update(self, symbol: str):
        ...
        if symbol not in self.cvd_history: self.cvd_history[symbol] = deque(maxlen=100)
        if self.cvd_history[symbol]: delta += self.cvd_history[symbol][-1][1]
    ...
    def get_divergence(self, symbol: str) -> Optional[str]:
        ...
        if len(candles_5m) < 10 or symbol not in self.cvd_history or len(self.cvd_history[symbol]) < 3: return None
        recent_price = [c.close for c in candles_5m[-3:]]
        recent_cvd = [v for _, v in list(self.cvd_history[symbol])[-3:]]
        if recent_price[-1] > recent_price[0] and recent_cvd[-1] < recent_cvd[0]: return "BEARISH"
        if recent_price[-1] < recent_price[0] and recent_cvd[-1] > recent_cvd[0]: return "BULLISH"
        return None
...
class SharpMoveDetector:
    ...
    def detect(self, symbol: str) -> Optional[Dict[str, Any]]:
        now = datetime.now(timezone.utc)
        if (now - self.last_alert).total_seconds() < self.alert_cooldown:
            return None
...
class OpenInterestEngine:
    def __init__(self, dm: MarketDataHub): self.dm = dm; self.oi_history: Dict[str, Deque[Tuple[datetime, float]]] = {}
    async def update(self, symbol: str):
        oi = await self.dm.get_open_interest(symbol)
        fr = await self.dm.get_funding_rate(symbol)
        if oi:
            if symbol not in self.oi_history: self.oi_history[symbol] = deque(maxlen=50)
...
# ---------- MARKET CONTEXT (2026 Calendar, Session, F&G) ----------
class MarketContextEngine:
    def __init__(self, dm: MarketDataHub):
        self.dm = dm; self.recent_trades: List[Dict] = []
        self.last_fng_update = datetime.min.replace(tzinfo=timezone.utc); self.cached_fng = 50
        self.economic_events = [
            ("2026-01-28", "19:00", "FOMC", 120), ("2026-03-18", "18:00", "FOMC", 120),
            ("2026-05-06", "18:00", "FOMC", 120), ("2026-06-17", "18:00", "FOMC", 120),
            ("2026-07-29", "18:00", "FOMC", 120), ("2026-09-16", "18:00", "FOMC", 120),
            ("2026-11-04", "19:00", "FOMC", 120), ("2026-12-09", "19:00", "FOMC", 120),
            ("2026-01-14", "13:30", "CPI", 90), ("2026-02-11", "13:30", "CPI", 90),
            ("2026-03-11", "12:30", "CPI", 90), ("2026-04-15", "12:30", "CPI", 90),
            ("2026-05-13", "12:30", "CPI", 90), ("2026-06-10", "12:30", "CPI", 90),
            ...
            ("2026-01-09", "13:30", "NFP", 90), ("2026-02-06", "13:30", "NFP", 90),
            ("2026-03-06", "13:30", "NFP", 90), ("2026-04-03", "12:30", "NFP", 90),
            ("2026-05-01", "12:30", "NFP", 90), ("2026-06-05", "12:30", "NFP", 90),
            ("2026-07-02", "12:30", "NFP", 90), ("2026-08-07", "12:30", "NFP", 90),
            ("2026-09-04", "12:30", "NFP", 90), ("2026-10-02", "12:30", "NFP", 90),
            ("2026-11-06", "13:30", "NFP", 90), ("2026-12-04", "13:30", "NFP", 90),
            ("2026-01-29", "13:30", "GDP", 90), ("2026-04-29", "12:30", "GDP", 90),
            ("2026-07-30", "12:30", "GDP", 90), ("2026-10-29", "12:30", "GDP", 90),
            ("2026-01-13", "13:30", "PPI", 60), ("2026-02-10", "13:30", "PPI", 60),
            ("2026-03-10", "12:30", "PPI", 60), ("2026-04-14", "12:30", "PPI", 60),
            ("2026-05-12", "12:30", "PPI", 60), ("2026-06-09", "12:30", "PPI", 60),
            ("2026-07-14", "12:30", "PPI", 60), ("2026-08-11", "12:30", "PPI", 60),
            ("2026-09-15", "12:30", "PPI", 60), ("2026-10-13", "12:30", "PPI", 60),
            ("2026-11-17", "13:30", "PPI", 60), ("2026-12-15", "13:30", "PPI", 60),
            ("2026-01-15", "13:30", "Retail Sales", 60), ("2026-02-13", "13:30", "Retail Sales", 60),
            ("2026-03-17", "12:30", "Retail Sales", 60), ("2026-04-14", "12:30", "Retail Sales", 60),
            ("2026-05-14", "12:30", "Retail Sales", 60), ("2026-06-16", "12:30", "Retail Sales", 60),
        ]

    async def get_fear_greed_index(self) -> int:
        now = datetime.now(timezone.utc)
        if (now - self.last_fng_update).total_seconds() < 3600:
            ...
            async with aiohttp.ClientSession() as sess:
                async with sess.get("https://api.alternative.me/fng/", timeout=10) as resp:
                    if resp.status == 200:
                        ...
                        logger.info(f"Fear & Greed Index: {self.cached_fng}")
        except Exception as e:
            logger.warning(f"Could not fetch F&G: {e}")
        return self.cached_fng
    ...
    def get_news_impact(self) -> Tuple[bool, int]:
        ...
        for event_date, event_time, event_name, impact_minutes in self.economic_events:
            if event_date == current_date:
                ...
                if -30 <= diff <= impact_minutes:
                    logger.info(f"⚠️ Near high-impact event: {event_name} ({abs(diff):.0f}min {'before' if diff < 0 else 'after'})")
    ...
    async def get_context_score(self) -> float:
        ...
        if is_news:
            if minutes_to_news < 0:
                logger.info("Pre-news blackout – no trading")
                return 0.0
            else:
                logger.info(f"Post-news recovery ({minutes_to_news}min) – reduced confidence")
                return 30.0
...
# ---------- LOSS PATTERN FILTER (persistent JSON) ----------
class LossPatternFilter:
    def __init__(self, filepath="loss_patterns.json"):
        self.filepath = filepath
        self.patterns: Dict[str, Dict[str, int]] = {}
        self._load()
    def _key(self, regime, direction, rsi_bucket, session, temp_bucket):
        return f"{regime}|{direction}|{rsi_bucket}|{session}|{temp_bucket}"
    def _bucket_rsi(self, rsi): return "low" if rsi < 35 else ("mid" if rsi < 65 else "high")
    def _bucket_temp(self, temp): return "cold" if temp < 40 else ("normal" if temp < 70 else "hot")
    def _load(self):
        try:
            with open(self.filepath, 'r') as f: self.patterns = json.load(f)
        except: self.patterns = {}
    def _save(self):
        try:
            with open(self.filepath, 'w') as f: json.dump(self.patterns, f)
        except Exception as e: logger.warning(f"Could not save loss patterns: {e}")
    def record(self, regime, direction, rsi, session, temperature, pnl):
        key = self._key(regime, direction, self._bucket_rsi(rsi), session, self._bucket_temp(temperature))
        if key not in self.patterns: self.patterns[key] = {"loss": 0, "win": 0}
        if pnl <= 0: self.patterns[key]["loss"] += 1
        else: self.patterns[key]["win"] += 1
        self._save()
    def should_reject(self, regime, direction, rsi, session, temperature) -> bool:
        key = self._key(regime, direction, self._bucket_rsi(rsi), session, self._bucket_temp(temperature))
        if key in self.patterns:
            stats = self.patterns[key]
            total = stats["loss"] + stats["win"]
            if total >= 2 and stats["loss"] / total > 0.6: return True
        return False
...
class KellyPositionSizer:
    def __init__(self, win_rate=0.55, avg_win=2.0, avg_loss=-1.0):
        self.win_rate = win_rate; self.avg_win = avg_win; self.avg_loss = abs(avg_loss); self.max_fraction = 0.02
...
class OnlineMLModel:
    ...
    def add_sample(self, features: np.ndarray, label: int):
        ...
        if len(self.feature_list) >= CONFIG["ML_MIN_SAMPLES"]:
            self._train()

    def _train(self):
        import xgboost as xgb
        if len(self.feature_list) < 10: return
        X = np.array(self.feature_list)
        y = np.array(self.label_list)
        try:
            ...
            self.trained = True
            logger.info(f"AI model trained on {len(X)} samples")
        except Exception as e:
...
# ---------- SIGNAL GENERATOR (with Adjusted Filters) ----------
class SignalGenerator:
    def __init__(self, dm: MarketDataHub):
        ...
        self.loss_filter = LossPatternFilter()
        self.ml_model = OnlineMLModel()
        self.last_signal = datetime.min.replace(tzinfo=timezone.utc)
        self.consecutive_losses = 0
        # Reversal cooldown
        self.last_reversal_direction = None
        self.last_reversal_time = datetime.min.replace(tzinfo=timezone.utc)
    ...
    async def generate(self, symbol: str) -> Optional[Signal]:
        ...
        logger.info(f"Market Context Score: {context_score:.1f}")
        if context_score < 30:
            logger.info(f"Market context too poor ({context_score:.1f}) – no signal")
            return None
        ...
        if sharp and sharp["detected"]:
            ...
            sl = entry - max(atr_5m * 1.2, min_sl_dist) if direction == "BUY" else entry + max(atr_5m * 1.2, min_sl_dist)
            ...
            avg_lat = self.dm.get_avg_latency()
            pos_size = CONFIG["DYNAMIC_RISK_MIN"]
            if avg_lat > 500: pos_size *= 0.2
            elif avg_lat > 200: pos_size *= 0.5
            pos_size *= (context_score / 100)
            sig = Signal(id=f"SOL_{now.strftime('%H%M%S')}_{random.randint(100, 999)}", symbol=symbol,
                         ...
                         confidence=70, timeframe="1m",
                         reason=f"SharpMove:{direction} VolSpike:{sharp['volume_spike']}",
                         whale_volume=0, timestamp=now, risk_per_trade=pos_size,
                         trade_type="SCALP", regime="SHARP")
        ...
        logger.info(f"Market Temperature: {temp:.1f}")
        if temp < 30:
            logger.info("Market too cold, no signal")
            return None

        rsi_5m = MathUtils.rsi([c.close for c in c5m])
        session = self.context_engine.get_session()

        # Loss pattern filter
        if self.loss_filter.should_reject(regime, mtf["trend_1h"], rsi_5m, session, temp):
            logger.info("Loss pattern filter rejected signal")
            return None

        # Reversal detection with Volume Filter
        reversal_signal = False
        reversal_direction = None
        volumes_5m = [c.volume for c in c5m[-20:]]
        avg_vol_5m = self.m.mean(volumes_5m) if volumes_5m else 0
        vol_ok_reversal = candles_5m[-1].volume >= avg_vol_5m * 1.2 if avg_vol_5m > 0 else False
        
        if rsi_5m < 35 and mtf["trend_1h"] == "BULLISH" and (regime == "TRENDING" or vol_ok_reversal):
            reversal_signal = True; reversal_direction = "BUY"
            logger.info(f"Reversal setup: RSI={rsi_5m:.1f}, 1h=UP → BUY (Vol:{'OK' if vol_ok_reversal else 'TRENDING'})")
        elif rsi_5m > 65 and mtf["trend_1h"] == "BEARISH" and (regime == "TRENDING" or vol_ok_reversal):
            reversal_signal = True; reversal_direction = "SELL"
            logger.info(f"Reversal setup: RSI={rsi_5m:.1f}, 1h=DOWN → SELL (Vol:{'OK' if vol_ok_reversal else 'TRENDING'})")

        # ---- Reversal cooldown for opposite direction ----
        if reversal_signal:
            if (self.last_reversal_direction is not None and
                self.last_reversal_direction != reversal_direction):
                if (now - self.last_reversal_time).total_seconds() < CONFIG["SIGNAL_COOLDOWN"]:
                    logger.info(f"Reversal {reversal_direction} blocked – opposite reversal {self.last_reversal_direction} just occurred")
                    reversal_signal = False
        ...
        if reversal_signal:
            ...
            atr_5m = self.m.atr([c.high for c in c5m], [c.low for c in c5m], [c.close for c in c5m])
            if atr_5m == 0: atr_5m = entry * 0.003
            min_sl_dist = entry * 0.0025
            sl = entry - max(atr_5m * 1.5, min_sl_dist) if direction == "BUY" else entry + max(atr_5m * 1.5, min_sl_dist)
            ...
            sig = Signal(id=f"SOL_{now.strftime('%H%M%S')}_{random.randint(100, 999)}", symbol=symbol,
                         direction=direction, entry_price=entry, stop_loss=sl, take_profit=[tp1, tp2, tp3],
                         confidence=50, timeframe="5m",
                         reason=f"Reversal:{direction} RSI:{rsi_5m:.1f}",
                         whale_volume=0, timestamp=now, risk_per_trade=pos_size,
                         trade_type="SCALP", regime=regime)
            # update reversal cooldown
            self.last_reversal_direction = reversal_direction
            self.last_reversal_time = now
            self.last_signal = now
            return sig

        # Breakout detection
        breakout_signal = False
        direction_break = None
        c5m_highs = [c.high for c in c5m[-20:]]
        c5m_lows = [c.low for c in c5m[-20:]]
        ...
        if avg_vol_break > 0:
            ...
            if vol_ratio_break > 2.0:
                if c5m[-1].close > max(c5m_highs[:-1]):
                    breakout_signal = True; direction_break = "BUY"
                elif c5m[-1].close < min(c5m_lows[:-1]):
                    breakout_signal = True; direction_break = "SELL"

        if regime == "RANGING" and not breakout_signal:
            prev_vols = [c.volume for c in c5m[-20:]]
            vol_spike = False
            if prev_vols:
                avg_vol = self.m.mean(prev_vols)
                vol_spike = c5m[-1].volume > avg_vol * 1.8
            ob_imb = self.ob.get_imbalance(symbol)
            if not vol_spike or ob_imb["score"] < 50:
                logger.info("RANGING & no strong volume/OB → no signal")
                return None

        if breakout_signal:
            direction = direction_break
            logger.info(f"Breakout detected: {direction}")
        else:
            if not aligned:
                logger.info("MTF not aligned, will reduce confidence")
            direction = "BUY" if mtf["trend_1h"] == "BULLISH" else "SELL"
        ...
        if prev_vols:
            avg_vol = self.m.mean(prev_vols)
            volume_spike = c5m[-1].volume > avg_vol * 1.5
        ...
        if struct_5m.get("bos") and struct_5m["bos"]["direction"] == direction:
            scores["structure"] += w["structure"] / 2
        ...
        if delta_bonus > 0:
            scores["delta"] = delta_bonus
            logger.info(f"Delta bonus +{delta_bonus}")
        elif delta_bonus < 0:
            scores["delta"] = delta_bonus
            logger.info(f"Delta penalty {delta_bonus}")

        cvd_div = self.cvd.get_divergence(symbol)
        if cvd_div:
            if cvd_div == "BULLISH" and direction == "BUY":
                scores["volume"] += 5
                logger.info("CVD Bullish divergence detected")
            elif cvd_div == "BEARISH" and direction == "SELL":
                scores["volume"] += 5
                logger.info("CVD Bearish divergence detected")

        poc = self.volume_profile.get_poc(symbol)
        if poc:
            dist_pct = abs(price - poc) / price * 100
            if dist_pct < 0.3:
                logger.info(f"Price near POC {poc:.2f}")

        oi_health = self.oi_engine.get_health(symbol)
        if oi_health["oi_trend"] == "UP" and direction == "BUY":
            scores["trend"] += 3
        elif oi_health["oi_trend"] == "DOWN" and direction == "SELL":
            scores["trend"] += 3
        if abs(oi_health["funding"]) > 0.005:
            logger.info(f"Extreme funding rate: {oi_health['funding']:.4f}")

        confidence = sum(scores.values())

        # Build features for AI
        features = np.array([
            mtf.get("strength", 50) / 100,
            1 if volume_spike else 0,
            whale["score"] / 100 if whale["detected"] else 0,
            ob_imb["score"] / 100,
            1 if liquidity["sweep"] else 0,
            1 if struct_5m["bos"] else 0,
            1 if regime == "TRENDING" else 0,
            temp / 100,
            rsi_5m / 100,
        ...
        if self.ml_model.trained:
            ml_proba = self.ml_model.predict_proba(features)
            logger.info(f"AI probability: {ml_proba:.2f}")
            if ml_proba < 0.45:
                logger.info("AI rejected signal")
                return None
            confidence = confidence * 0.4 + ml_proba * 100 * 0.6
        else:
            ml_proba = 0.5

        if not breakout_signal and not aligned:
            confidence *= 0.8
            logger.info(f"Confidence reduced due to MTF misalignment: {confidence:.1f}")

        if breakout_signal and confidence < 45:
            confidence = 45

        confidence *= (context_score / 100)
        logger.info(f"Confidence after context adjustment: {confidence:.1f}")

        logger.info(f"SCORE BREAKDOWN {symbol}: {scores} | confidence={confidence:.0f}")
        ...
        if direction == "BUY":
            ...
            tp1 = entry + atr_5m * 1.5
            tp2 = entry + atr_5m * 2.5
            tp3 = entry + atr_5m * 4.0
        else:
            sl = entry + max(atr_5m * 1.8, min_sl_dist)
            tp1 = entry - atr_5m * 1.5
            tp2 = entry - atr_5m * 2.5
        ...
        if avg_lat > 500:
            dynamic_risk *= 0.2
            logger.info(f"High latency {avg_lat:.0f}ms, position reduced to 20%")
        elif avg_lat > 200:
            dynamic_risk *= 0.5
            logger.info(f"Moderate latency {avg_lat:.0f}ms, position reduced to 50%")
        ...
        sig = Signal(id=f"SOL_{now.strftime('%H%M%S')}_{random.randint(100, 999)}", symbol=symbol,
                     ...
                     reason=f"Trend:{direction} Regime:{regime} Temp:{temp:.0f} AI:{ml_proba:.2f}",
        ...
        # Attach features for later training
        sig.features = features
        return sig

    def report_result(self, pnl: float, features: np.ndarray, regime: str, direction: str, rsi: float, session: str, temperature: float):
        if pnl <= 0:
        ...
        self.loss_filter.record(regime, direction, rsi, session, temperature, pnl)
        # AI training
        label = 1 if pnl > 0 else 0
        self.ml_model.add_sample(features, label)


# ---------- FULL MARKET MENTOR ENGINE (17 methods, identical to previous full version) ----------
class MarketMentorEngine:
    ...
    async def full_analysis(self, symbol: str) -> str:
        ...
        bba = await self.dm.get_best_bid_ask(symbol)
        if bba["spread"] > CONFIG["MAX_SPREAD_PCT"]: return "🚫 اسپرد بالاست، الان زمان ورود نیست."
        candles_1h = list(self.dm.candle_buffers[symbol].get("1H", []))
        candles_5m = list(self.dm.candle_buffers[symbol].get("5m", []))
        if len(candles_5m) < 20 or len(candles_1h) < 20: return "⏳ اطلاعات کندل کافی نیست. کمی صبر کن."
        closes_1h = [c.close for c in candles_1h]
        ema20 = self.m.ema(closes_1h, 20); ema50 = self.m.ema(closes_1h, 50)
        if not ema20 or not ema50: return "📊 داده‌های EMA ناکافی است."
        ...
        rsi_desc = ("فشار خرید (اشباع خرید)" if rsi > 65 else "فشار فروش (اشباع فروش)" if rsi < 35 else
                    "رنج / خنثی" if 40 <= rsi <= 60 else "متمایل به خرید" if rsi > 60 else "متمایل به فروش")
        ob = self.dm.orderbook_snapshots.get(symbol, {"bids": [], "asks": []})
        bid_liq = sum(q for _, q in ob.get("bids", [])[:20]) if ob.get("bids") else 0
        ask_liq = sum(q for _, q in ob.get("asks", [])[:20]) if ob.get("asks") else 0
        total = bid_liq + ask_liq
        pressure = ("خرید 🟢" if (total > 0 and bid_liq / total > 0.6) else
                    "فروش 🔴" if (total > 0 and bid_liq / total < 0.4) else "متعادل ⚖️")
        trades = list(self.dm.trade_buffers.get(symbol, []))
        cutoff = datetime.now(timezone.utc) - timedelta(minutes=5)
        large = [t for t in trades if t["time"] >= cutoff and t["price"] * t["qty"] >= 10000]
        whale = "فعال 🐋" if len(large) >= 2 else "ساکت"
        decision = "ورود نکن – شرایط قطعی نیست"
        if trend.startswith("صعودی") and pressure == "خرید 🟢" and rsi < 65:
            decision = "اسکالپ لانگ پیشنهاد می‌شود 🟢"
        elif trend.startswith("نزولی") and pressure == "فروش 🔴" and rsi > 35:
            decision = "اسکالپ شورت پیشنهاد می‌شود 🔴"
        return (f"📊 تحلیل SOL-USDT-SWAP (قیمت: {price:.4f})\n"
                f"• روند ۱ ساعته: {trend}\n"
                f"• نوسان: {vol_desc}\n"
                f"• RSI (5m): {rsi:.1f} – {rsi_desc}\n"
                f"• فشار دفتر سفارش: {pressure}\n"
                f"• نهنگ‌ها: {whale}\n\n"
                f"💡 پیشنهاد: {decision}")
    ...
    async def scalp_analysis(self, symbol: str) -> str:
        ...
        if rsi > 60 and atr/price > 0.002 and vol_ok and trend_5m != "BEARISH":
            if rsi > 70: return f"📊 اسکالپ SOL – RSI در اشباع خرید ({rsi:.1f}) ⚠️\nمنتظر پول‌بک باش، الان لانگ نکن."
            direction = "BUY"
        elif rsi < 40 and atr/price > 0.002 and vol_ok and trend_5m != "BULLISH":
            if rsi < 30: return f"📊 اسکالپ SOL – RSI در اشباع فروش ({rsi:.1f}) ⚠️\nمنتظر پول‌بک باش، الان شورت نکن."
            direction = "SELL"
        if not direction and rsi < 35 and trend_1h == "BULLISH":
            direction = "BUY"
        elif not direction and rsi > 65 and trend_1h == "BEARISH":
            direction = "SELL"

        if not direction:
            reason = "حجم پایین" if not vol_ok else "خلاف روند" if (rsi>60 and trend_5m=="BEARISH") or (rsi<40 and trend_5m=="BULLISH") else "شرایط اسکالپ ضعیف"
            return (f"📊 اسکالپ SOL (قیمت: {price:.4f})\n• RSI: {rsi:.1f}\n• ATR: {atr:.4f}\n• حجم: {'کافی' if vol_ok else 'کم'}\n• روند ۵m: {trend_5m}\n💡 {reason} – اسکالپ نزن")
        warning = ""
        if (direction == "BUY" and trend_1h == "BEARISH") or (direction == "SELL" and trend_1h == "BULLISH"):
            warning = f"\n⚠️ توجه: روند ۱h {trend_1h} – اسکالپ خلاف روند اصلیه. با حجم کمتر وارد شو."
        if direction == "BUY":
            ...
            swing_high = max(highs[-10:]) if len(highs) >= 10 else price + atr
        ...
        return (f"⚡ سیگنال اسکالپ SOL {'🟢' if direction=='BUY' else '🔴'}\n"
                f"🎯 ورود: {entry:.4f}\n🛑 حد ضرر: {sl:.4f}\n"
                f"✅ اهداف:\n   TP1: {tp1:.4f}\n   TP2: {tp2:.4f}\n   TP3: {tp3:.4f}\n"
                f"📊 RSI:{rsi:.1f} | ATR:{atr:.4f} | حجم:{'کافی' if vol_ok else 'کم'} | روند:{trend_5m}"
                f"{warning}")
    ...
    async def swing_analysis(self, symbol: str) -> str:
        ...
        if rsi_1h > 70 or rsi_1h < 30:
            return f"📈 میان‌مدت SOL – RSI 1h در اشباع ({rsi_1h:.1f}) ⚠️\nبهتره صبر کنی تا از ناحیه اشباع خارج بشه."
        ema20_1h = self.m.ema(closes_1h, 20); ema50_1h = self.m.ema(closes_1h, 50)
        if not ema20_1h or not ema50_1h: return "📊 EMA ناکافی"
        trend_1h = "BULLISH" if ema20_1h[-1] > ema50_1h[-1] else "BEARISH"
        avg_vol_1h = self.m.mean([c.volume for c in candles_1h[-10:]]) if len(candles_1h) >= 10 else 0
        vol_weak = candles_1h[-1].volume < avg_vol_1h * 0.7 if avg_vol_1h > 0 else False
        direction = trend_1h
        if direction == "BULLISH":
            sl = min(lows_1h[-10:]) if len(lows_1h) >= 10 else price * 0.97
            entry = price; risk = entry - sl
            tp1 = entry + risk * 1.0; tp2 = entry + risk * 2.0; tp3 = entry + risk * 3.0
        else:
            sl = max(highs_1h[-10:]) if len(highs_1h) >= 10 else price * 1.03
            entry = price; risk = sl - entry
            tp1 = entry - risk * 1.0; tp2 = entry - risk * 2.0; tp3 = entry - risk * 3.0
        if risk <= 0: return "⚠️ خطا در محاسبه ریسک."
        warning = []
        if vol_weak: warning.append("⚠️ حجم ۱h پایین‌تر از میانگین – روند ضعیفه")
        dist_ema = (price - ema50_1h[-1]) / ema50_1h[-1] * 100
        if abs(dist_ema) > 5: warning.append(f"⚠️ فاصله زیاد از EMA50 ({dist_ema:.1f}%) – احتمال Pullback")
        return (f"📈 سیگنال میان‌مدت SOL {'🟢' if direction=='BULLISH' else '🔴'}\n"
                f"🎯 ورود: {entry:.4f}\n🛑 حد ضرر: {sl:.4f}\n"
                f"✅ اهداف:\n   TP1: {tp1:.4f}\n   TP2: {tp2:.4f}\n   TP3: {tp3:.4f}\n"
                f"📊 روند ۱h: {trend_1h}\n"
    ...
    async def liquidity_status(self, symbol: str) -> str:
        ...
        return (f"📊 بررسی نقدینگی SOL\n"
                f"• حجم ۲۴h: ${liq:,.0f} (حداقل: ${min_liq:,.0f}) {'✔️' if liq_ok else '❌'}\n"
    ...
    async def daily_suggestion(self, symbol: str) -> str:
        ...
        if not price: return "❌ قیمت در دسترس نیست."
        candles_1h = list(self.dm.candle_buffers[symbol].get("1H", []))
        if len(candles_1h) < 20: return "⏳ کندل ۱h کافی برای پیشنهاد روزانه نیست."
        closes_1h = [c.close for c in candles_1h]
        ema20_1h = self.m.ema(closes_1h, 20); ema50_1h = self.m.ema(closes_1h, 50)
        if not ema20_1h or not ema50_1h: return "📊 EMA ناکافی"
        trend_1h = "BULLISH" if ema20_1h[-1] > ema50_1h[-1] else "BEARISH"
        atr_1h = MathUtils.atr([c.high for c in candles_1h], [c.low for c in candles_1h], closes_1h)
        if atr_1h == 0: atr_1h = price * 0.005
        vol_condition = "بالا" if atr_1h > 0.01 * price else "پایین" if atr_1h < 0.005 * price else "متوسط"
        bba = await self.dm.get_best_bid_ask(symbol); spread = bba["spread"]
        spread_ok = spread <= CONFIG["MAX_SPREAD_PCT"]
        if not spread_ok: suggestion = "⚠️ اسپرد بالاست، امروز ورود رو محدود کن."
        else:
            suggestion = ("📈 روند صعودی + اسپرد خوب. امروز فرصت‌های لانگ جذابن." if trend_1h == "BULLISH"
                          else "📉 روند نزولی + اسپرد خوب. امروز شورت‌ها شانس موفقیت بیشتری دارن.")
        return (f"📅 پیشنهاد روز SOL (قیمت: {price:.4f})\n"
                f"• روند ۱h: {trend_1h}\n"
                f"• نوسان: {vol_condition}\n"
                f"• اسپرد: {spread:.2f}% {'✔️' if spread_ok else '❌'}\n💡 {suggestion}")

    async def fakeout_detector(self, symbol: str) -> str:
        candles_5m = list(self.dm.candle_buffers[symbol].get("5m", []))
        if len(candles_5m) < CONFIG["STRUCTURE_SWING_PERIOD"] * 2 + 2: return "⏳ کندل کافی برای تشخیص فیک‌اوut نیست."
        last = candles_5m[-1]; recent_high = max(c.high for c in candles_5m[-10:-1]); recent_low = min(c.low for c in candles_5m[-10:-1])
        ...
        if last.high > recent_high and last.volume < avg_vol * 0.8 and last.upper_shadow > last.body_size * 0.8:
            return (f"⚠️ هشدار فیک بریک‌اوت!\n"
                    f"• کندل صعودی از سقف {recent_high:.4f} عبور کرده ولی:\n"
                    f"• حجم پایینه ({last.volume:.1f} vs میانگین {avg_vol:.1f})\n"
                    f"• سایه بالایی بلنده → احتمال برگشت بالاست.\n💡 پیشنهاد: فعلاً وارد پوزیشن نشو.")
        if last.low < recent_low and last.volume < avg_vol * 0.8 and last.lower_shadow > last.body_size * 0.8:
            return (f"⚠️ هشدار فیک بریک‌اوت!\n"
                    f"• کندل نزولی از کف {recent_low:.4f} عبور کرده ولی:\n"
                    f"• حجم پایینه ({last.volume:.1f} vs میانگین {avg_vol:.1f})\n"
                    f"• سایه پایینی بلنده → احتمال برگشت بالاست.\n💡 پیشنهاد: فعلاً وارد پوزیشن نشو.")
        return "✅ در حال حاضر نشانه‌ای از فیک بریک‌اوت دیده نمی‌شه."
    ...
    async def daily_summary(self, closed_trades: List[Trade]) -> str:
        ...
        return (f"📆 خلاصه امروز SOL\n"
                f"• معاملات بسته شده: {len(today_trades)}\n"
                f"• برد: {len(wins)} | باخت: {len(losses)}\n"
                f"• نرخ برد: {win_rate:.1f}%\n"
                f"• سود/ضرر خالص: {total_pnl:.2f}%\n"
                f"• میانگین سود: {avg_win:.2f}% | ضرر: {avg_loss:.2f}%")
    ...
    async def key_levels(self, symbol: str) -> str:
        ...
        candles_1h = list(self.dm.candle_buffers[symbol].get("1H", []))
        candles_15m = list(self.dm.candle_buffers[symbol].get("15m", []))
        if len(candles_1h) < 20: return "⏳ کندل ۱h کافی نیست."
        res_1h, sup_1h = swing_points(candles_1h)
        res_15m, sup_15m = [], []
        if len(candles_15m) >= 20: res_15m, sup_15m = swing_points(candles_15m, period=3)
        res_all = res_1h + res_15m; sup_all = sup_1h + sup_15m
        res_count = Counter(res_all); sup_count = Counter(sup_all)
        sorted_res = sorted(res_count.items(), key=lambda x: (-x[1], -x[0]))[:5]
        sorted_sup = sorted(sup_count.items(), key=lambda x: (-x[1], x[0]))[:5]
        lines = [f"📊 سطوح کلیدی SOL (قیمت فعلی: {price:.4f})"]
        if sorted_res:
            res_str = " | ".join(f"{p:.4f}(x{c})" for p, c in sorted_res)
            lines.append(f"🔴 مقاومت‌ها: {res_str}")
        if sorted_sup:
            sup_str = " | ".join(f"{p:.4f}(x{c})" for p, c in sorted_sup)
            lines.append(f"🟢 حمایت‌ها: {sup_str}")
        for p, c in sorted_sup:
            if price <= p + 0.15 and price >= p - 0.05:
                lines.append(f"⚡ قیمت نزدیک حمایت {p:.4f} (تست شده {c} بار) – فرصت لانگ")
                break
        for p, c in sorted_res:
            if price >= p - 0.15 and price <= p + 0.05:
                lines.append(f"⚡ قیمت نزدیک مقاومت {p:.4f} (تست شده {c} بار) – فرصت شورت")
                break
        return "\n".join(lines)

    async def divergence_detector(self, symbol: str) -> str:
        candles_5m = list(self.dm.candle_buffers[symbol].get("5m", []))
        if len(candles_5m) < 40: return "⏳ کندل کافی برای تشخیص واگرایی نیست (حداقل ۴۰ کندل)."
        ...
        if len(p_highs) >= 2 and len(r_highs) >= 2:
            if closes[p_highs[-1]] > closes[p_highs[-2]] and rsi_vals[r_highs[-1]] < rsi_vals[r_highs[-2]]:
                return "🔴 واگرایی مخفی نزولی (Regular Bearish Divergence) شناسایی شد!\n• قیمت قله بالاتر، RSI قله پایین‌تر → احتمال برگشت نزولی."
        if len(p_lows) >= 2 and len(r_lows) >= 2:
            if closes[p_lows[-1]] < closes[p_lows[-2]] and rsi_vals[r_lows[-1]] > rsi_vals[r_lows[-2]]:
                return "🟢 واگرایی مخفی صعودی (Regular Bullish Divergence) شناسایی شد!\n• قیمت کف پایین‌تر، RSI کف بالاتر → احتمال برگشت صعودی."
        return "✅ واگرایی مشخصی در ۵m دیده نمی‌شه."

    async def orderbook_walls(self, symbol: str) -> str:
        ob = self.dm.orderbook_snapshots.get(symbol, {"bids": [], "asks": []})
        bids = ob.get("bids", []); asks = ob.get("asks", [])
        if not bids or not asks: return "❌ اطلاعات دفتر سفارشات در دسترس نیست."
        total_bid_vol = sum(q for _, q in bids); total_ask_vol = sum(q for _, q in asks)
        ...
        price = await self.dm.get_current_price(symbol)
        if max_bid:
            pct = max_bid[1]/total_bid_vol*100 if total_bid_vol else 0
            dist = price - max_bid[0] if price else 0
            lines.append(f"🟢 دیوار خرید: {max_bid[0]:.4f} (حجم {max_bid[1]:.2f} SOL, {pct:.1f}% از عمق خرید, فاصله {dist:.2f})")
            if pct > 40: lines.append("⚡ این دیوار >40% عمقه – حمایت قوی!")
        if max_ask:
            pct = max_ask[1]/total_ask_vol*100 if total_ask_vol else 0
            dist = max_ask[0] - price if price else 0
            lines.append(f"🔴 دیوار فروش: {max_ask[0]:.4f} (حجم {max_ask[1]:.2f} SOL, {pct:.1f}% از عمق فروش, فاصله {dist:.2f})")
    ...
    async def expected_range(self, symbol: str) -> str:
        ...
        bias = ""
        if ema20 and ema50:
            if ema20[-1] > ema50[-1]: bias = "• احتمال حرکت به سمت سقف محدوده بیشتره (روند صعودی)"
            elif ema20[-1] < ema50[-1]: bias = "• احتمال حرکت به سمت کف محدوده بیشتره (روند نزولی)"
        return (f"📈 محدوده‌ی انتظاری کندل بعدی (5m):\n• سقف: {upper:.4f}\n• کف: {lower:.4f}\n• محدوده ~{atr*1.4:.4f} دلار\n{bias}")

    async def orderflow_summary(self, symbol: str) -> str:
        trades = list(self.dm.trade_buffers.get(symbol, []))
        now = datetime.now(timezone.utc)
        recent = [t for t in trades if t["time"] >= now - timedelta(minutes=5)]
        prev = [t for t in trades if now - timedelta(minutes=35) <= t["time"] < now - timedelta(minutes=5)]
        if not recent: return "⏳ در ۵ دقیقه اخیر تریدی ثبت نشده."
        def calc_pct(tlist):
            buy = sum(t["qty"] for t in tlist if not t["is_buyer_maker"])
            sell = sum(t["qty"] for t in tlist if t["is_buyer_maker"])
            total = buy + sell
            if total == 0: return 50, 50
            return buy/total*100, sell/total*100
        buy_now, sell_now = calc_pct(recent)
        buy_prev, sell_prev = calc_pct(prev) if prev else (buy_now, sell_now)
        delta = buy_now - buy_prev
        change = f"تغییر: {'افزایش' if delta>5 else 'کاهش' if delta<-5 else 'ثابت'} فشار خرید ({delta:+.1f}%)"
        summary = "فشار خرید غالب" if buy_now>55 else "فشار فروش غالب" if sell_now>55 else "متعادل"
        return (f"📊 جریان سفارشات:\n• الان (۵دقیقه): خرید {buy_now:.1f}% / فروش {sell_now:.1f}%\n• ۳۰ دقیقه قبل: خرید {buy_prev:.1f}%\n• {change}\n• جمع‌بندی: {summary}")
    ...
    async def mtf_heatmap(self, symbol: str) -> str:
        ...
        for label, tf in tfs.items():
            ...
            if len(candles) < 20:
                lines.append(f"{tf} ⚪ داده ناکافی")
                continue
            closes = [c.close for c in candles]
            ema20 = self.m.ema(closes, 20); ema50 = self.m.ema(closes, 50)
            rsi = MathUtils.rsi(closes)
            direction = "🟢" if (ema20 and ema50 and ema20[-1] > ema50[-1]) else "🔴"
            lines.append(f"{tf} {direction} RSI:{rsi:.1f}")
        has_1h_red = "1H 🔴" in "\n".join(lines)
        has_5m_green = "5m 🟢" in "\n".join(lines)
        has_15m_green = "15m 🟢" in "\n".join(lines)
        if has_1h_red and (has_5m_green or has_15m_green):
            lines.append("⚠️ ناهماهنگی: تایم بالا نزولی، پایین صعودی – اسکالپ محتاطانه.")
        elif not has_1h_red and "5m 🔴" in "\n".join(lines):
            lines.append("⚠️ ناهماهنگی: تایم بالا صعودی، پایین نزولی – اسکالپ محتاطانه.")
        return "\n".join(lines)
    ...
    async def pending_setup(self, symbol: str) -> str:
        ...
        def swing_points(candles, period=5):
            ...
            res = []; sup = []
            for i in range(period, len(highs)-period):
                if highs[i] == max(highs[i-period:i+period+1]): res.append(highs[i])
                if lows[i] == min(lows[i-period:i+period+1]): sup.append(lows[i])
            return sorted(set(res), reverse=True), sorted(set(sup))
        candles_1h = list(self.dm.candle_buffers[symbol].get("1H", []))
        candles_15m = list(self.dm.candle_buffers[symbol].get("15m", []))
        res_all, sup_all = [], []
        if len(candles_1h) >= 20: r,s = swing_points(candles_1h); res_all.extend(r); sup_all.extend(s)
        if len(candles_15m) >= 20: r,s = swing_points(candles_15m, period=3); res_all.extend(r); sup_all.extend(s)
        if not res_all and not sup_all: return "⛔ سطح معتبری پیدا نشد."
        res_all = sorted(set(res_all), reverse=True); sup_all = sorted(set(sup_all))
        long_level = next((s for s in sup_all if s < price and (price-s)/price*100 >= 0.8), None)
        short_level = next((r for r in res_all if r > price and (r-price)/price*100 >= 0.8), None)
        if not long_level and not short_level: return "⛔ سطحی با حداقل ۰.۸٪ فاصله یافت نشد."
        trend = "NEUTRAL"
        if len(candles_1h) >= 20:
            closes = [c.close for c in candles_1h]; ema20=self.m.ema(closes,20); ema50=self.m.ema(closes,50)
            if ema20 and ema50: trend = "BULLISH" if ema20[-1]>ema50[-1] else "BEARISH"
        atr_1h = MathUtils.atr([c.high for c in candles_1h], [c.low for c in candles_1h], [c.close for c in candles_1h]) if len(candles_1h)>=14 else price*0.005
        suggestions=[]
        if long_level and trend in ("BULLISH","NEUTRAL"):
            entry=long_level; sl=entry-atr_1h*0.5; risk=entry-sl
            tp1=entry+risk*2; tp2=entry+risk*3; tp3=entry+risk*5
            suggestions.append(f"🟢 پندینگ لانگ روزانه در {entry:.4f}\n🛑 SL:{sl:.4f}\n✅ TP:{tp1:.4f}/{tp2:.4f}/{tp3:.4f}\nفاصله:{(price-entry)/price*100:.2f}%")
        if short_level and trend in ("BEARISH","NEUTRAL"):
            entry=short_level; sl=entry+atr_1h*0.5; risk=sl-entry
            tp1=entry-risk*2; tp2=entry-risk*3; tp3=entry-risk*5
            suggestions.append(f"🔴 پندینگ شورت روزانه در {entry:.4f}\n🛑 SL:{sl:.4f}\n✅ TP:{tp1:.4f}/{tp2:.4f}/{tp3:.4f}\nفاصله:{(entry-price)/price*100:.2f}%")
        header=f"🎯 پندینگ روزانه SOL ({price:.4f})\nروند: {trend}\n"
        return header + "\n\n".join(suggestions)
    ...
    async def pending_setup_scalp(self, symbol: str) -> str:
        ...
        if not price: return "❌ قیمت در دسترس نیست."
        bba = await self.dm.get_best_bid_ask(symbol)
        if bba["spread"] > 0.05: return "🚫 اسپرد بالای 0.05%، پندینگ اسکالپ توجیه نداره."
        candles_5m = list(self.dm.candle_buffers[symbol].get("5m", []))
        if len(candles_5m) < 20: return "⏳ کندل ۵m کافی نیست."
        closes = [c.close for c in candles_5m]; highs = [c.high for c in candles_5m]; lows = [c.low for c in candles_5m]
        atr = MathUtils.atr(highs, lows, closes)
        ...
        trend = "NEUTRAL"
        if ema20 and ema50: trend = "BULLISH" if ema20[-1] > ema50[-1] else "BEARISH"
        def swings(arr, period=3):
            res = []; sup = []
            for i in range(period, len(arr)-period):
                if arr[i] == max(arr[i-period:i+period+1]): res.append(arr[i])
                if arr[i] == min(arr[i-period:i+period+1]): sup.append(arr[i])
            return res, sup
        res_highs, _ = swings(highs, period=3)
        _, sup_lows = swings(lows, period=3)
        res = sorted(set(res_highs), reverse=True)
        sup = sorted(set(sup_lows))
        valid_long = next((s for s in sup if s < price and 0.003 <= (price-s)/price <= 0.008), None)
        valid_short = next((r for r in res if r > price and 0.003 <= (r-price)/price <= 0.008), None)
        if not valid_long and not valid_short:
            return "🎯 پندینگ اسکالپ: در حال حاضر سطحی با فاصله ۰.۳٪ تا ۰.۸٪ وجود نداره. صبر کن."
        suggestions = []
        if valid_long and trend != "BEARISH":
            entry = valid_long
            stop_distance = max(atr * 1.0, price * 0.003)
            sl = entry - stop_distance
            risk = entry - sl
            tp1 = entry + risk * 1.5
            tp2 = entry + risk * 2.5
            tp3 = entry + risk * 3.5
            suggestions.append(f"🟢 اسکالپ لانگ در {entry:.4f}\n🛑 SL:{sl:.4f}\n✅ TP1:{tp1:.4f} TP2:{tp2:.4f} TP3:{tp3:.4f}\nفاصله:{(price-entry)/price*100:.2f}% | ریسک:{risk:.4f}")
        if valid_short and trend != "BULLISH":
            entry = valid_short
            stop_distance = max(atr * 1.0, price * 0.003)
            sl = entry + stop_distance
            risk = sl - entry
            ...
            tp2 = entry - risk * 2.5
            tp3 = entry - risk * 3.5
            suggestions.append(f"🔴 اسکالپ شورت در {entry:.4f}\n🛑 SL:{sl:.4f}\n✅ TP1:{tp1:.4f} TP2:{tp2:.4f} TP3:{tp3:.4f}\nفاصله:{(entry-price)/price*100:.2f}% | ریسک:{risk:.4f}")
        if not suggestions:
            return "🎯 پندینگ اسکالپ: جهت مخالف روند ۵m، پیشنهادی ندارم."
        return f"🎯 پندینگ اسکالپ SOL ({price:.4f})\n• روند ۵m: {trend}\n• ATR:{atr:.4f}\n\n" + "\n\n".join(suggestions)
    ...
    async def full_report(self, symbol: str) -> str:
        ...
        heatmap = await self.mtf_heatmap(symbol)
        parts.append(heatmap)
        parts.append(await self.fakeout_detector(symbol))
        parts.append(await self.pending_setup_scalp(symbol))
        return "\n\n".join(parts)
...
class TradeManager:
    ...
    def add(self, sig: Signal):
        ...
        t.trailing_stop = sig.stop_loss
        t.peak_price = sig.entry_price
        t.features = getattr(sig, 'features', None)
        t.regime = sig.regime
        ...
        while True:
            ...
            for symbol in CONFIG["SYMBOLS"]:
                ...
                for tid, t in list(self.active.items()):
                    ...
                    if t.direction == "BUY":
                        if p <= t.stop_loss:
                            ...
                            continue
                        if p >= t.tp1:
                            new_stop = max(t.trailing_stop, p - atr * 1.2)
                        ...
                        if p >= t.stop_loss:
                            ...
                            continue
                        if p <= t.tp1:
                            new_stop = min(t.trailing_stop, p + atr * 1.2)
                            t.trailing_stop = new_stop
                            if p >= t.trailing_stop:
                                t.status = "CLOSED"; t.exit_price = p
                                t.pnl_pct = (t.entry_price - p) / t.entry_price * 100
                ...
                for tid in closed:
                    ...
                    if t_closed:
                        if self.signal_gen:
                            rsi = MathUtils.rsi([c.close for c in c5m]) if len(c5m) >= 14 else 50
                            session = self.signal_gen.context_engine.get_session()
                            temp = await self.signal_gen.market_temp.get_temperature(symbol)
                            self.signal_gen.report_result(
                                t_closed.pnl_pct,
                                features=t_closed.features if t_closed.features is not None else np.zeros(10),
                                regime=t_closed.regime if t_closed.regime else "UNKNOWN",
                                direction=t_closed.direction,
                                rsi=rsi,
                                session=session,
                                temperature=temp
                            )
                        if self.on_close_callback:
                            asyncio.create_task(self.on_close_callback(t_closed))
...
if TELEGRAM_AVAILABLE:
    class TelegramBot:
        ...
        async def startup(self): await self._send("🚀 OKX SOL Scalper started (AI ready)")
        ...
        async def _start_command(self, update, context):
            ...
            await update.message.reply_text("🤖 به ربات تحلیل SOL خوش اومدی!\nیکی از دکمه‌های زیر رو بزن:", reply_markup=reply_markup)

        async def _handle_buttons(self, update, context):
            if not update.message or not update.message.text: return
            text = update.message.text
            if text == "📊 تحلیل کامل": await self._mentor_command(update, context)
            elif text == "⚡ اسکالپ": await self._scalp_command(update, context)
            elif text == "📈 میان‌مدت": await self._swing_command(update, context)
            elif text == "💧 نقدینگی": await self._liquidity_command(update, context)
            elif text == "📅 پیشنهاد روز": await self._daily_suggestion_command(update, context)
            elif text == "⚠️ فیک بریک‌اوت": await self._fakeout_command(update, context)
            elif text == "📆 خلاصه امروز": await self._daily_summary_command(update, context)
            elif text == "❤️ سلامت ربات": await self._health_command(update, context)
            elif text == "📋 معاملات باز": await self._trades_command(update, context)
            elif text == "🏔 سطوح کلیدی": await self._key_levels_command(update, context)
            elif text == "🔄 واگرایی": await self._divergence_command(update, context)
            elif text == "🧱 دیوارهای سفارش": await self._orderbook_walls_command(update, context)
            elif text == "📏 محدوده انتظاری": await self._expected_range_command(update, context)
            elif text == "🌊 جریان سفارشات": await self._orderflow_command(update, context)
            elif text == "🎯 هماهنگی تایم‌فریم‌ها": await self._mtf_heatmap_command(update, context)
            elif text == "⏳ پندینگ روزانه": await self._pending_setup_command(update, context)
            elif text == "⚡ پندینگ اسکالپ": await self._pending_scalp_command(update, context)
            elif text == "📋 گزارش کامل": await self._full_report_command(update, context)

        async def _mentor_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.full_analysis("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _scalp_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.scalp_analysis("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _swing_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.swing_analysis("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _liquidity_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.liquidity_status("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _health_command(self, update, context):
            dm = self.mentor.dm if self.mentor else None
            if not dm: return
            msg = (f"📊 وضعیت ربات\n• درخواست‌های API: {dm.request_count}\n• خطاها: {dm.error_count}\n"
                   f"• تریدهای بافر: {len(dm.trade_buffers['SOL-USDT-SWAP'])}\n"
                   f"• کندل‌های 5m: {len(dm.candle_buffers['SOL-USDT-SWAP'].get('5m',[]))}\n• WebSocket: خاموش (REST mode)")
        ...
        async def _daily_suggestion_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.daily_suggestion("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _fakeout_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.fakeout_detector("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _daily_summary_command(self, update, context):
            closed = self.closed_trades if hasattr(self, 'closed_trades') else []
            if not self.mentor: return
            res = await self.mentor.daily_summary(closed); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _key_levels_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.key_levels("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _divergence_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.divergence_detector("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _orderbook_walls_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.orderbook_walls("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _expected_range_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.expected_range("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _orderflow_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.orderflow_summary("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _mtf_heatmap_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.mtf_heatmap("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _pending_setup_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.pending_setup("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        async def _pending_scalp_command(self, update, context):
            if not self.mentor: return
            res = await self.mentor.pending_setup_scalp("SOL-USDT-SWAP"); await update.message.reply_text(res, parse_mode=ParseMode.MARKDOWN)
...
# ---------- REAL-TIME ENGINE (REST polling) ----------
class RealTimeMarketEngine:
    def __init__(self, dm: MarketDataHub, pipeline):
        self.dm = dm; self.pipeline = pipeline
        self.pipeline_locks = {sym: asyncio.Lock() for sym in CONFIG["SYMBOLS"]}
        self.symbol = CONFIG["SYMBOLS"][0]
        self.tfs = CONFIG["TIMEFRAMES"]["scalp"] + CONFIG["TIMEFRAMES"]["midterm"]

    async def run(self):
        while True:
            try: await self._poll_all()
    ...
    async def _poll_all(self):
        ...
        for tf in self.tfs:
            try:
                candles = await self.dm.get_klines(sym, tf, limit=1)
                if candles: self.dm.candle_buffers[sym][tf].append(candles[0])
            except Exception as e: logger.warning(f"Failed to fetch {tf} candles: {e}")
        try:
            await self.dm.get_order_book(sym)
            await self.dm.get_agg_trades(sym)
            await self.dm.get_open_interest(sym)
            await self.dm.get_funding_rate(sym)
        except Exception as e: logger.warning(f"Failed to fetch orderbook/trades: {e}")
        if not self.pipeline_locks[sym].locked():
            asyncio.create_task(self._run_pipeline(sym))

    async def _run_pipeline(self, symbol):
        async with self.pipeline_locks[symbol]:
            await self.pipeline(symbol)


# ---------- MAIN BOT ----------
class UltimateOKXBot:
    def __init__(self):
        ...
        self.engine = RealTimeMarketEngine(self.dm, self._callback)
        self.is_running = False
        self.health_server = None

    async def _callback(self, symbol: str):
        try:
            if not await self.risk.can_open(): return
    ...
    async def _periodic(self):
        while self.is_running:
            ...
            for symbol in CONFIG["SYMBOLS"]:
                logger.info(f"Health | req:{self.dm.request_count} err:{self.dm.error_count} trades:{len(self.dm.trade_buffers[symbol])}")
                asyncio.create_task(self._callback(symbol))
    ...
    async def start_health_server(self):
        ...
        await runner.setup()
        site = web.TCPSite(runner, '0.0.0.0', CONFIG["HEALTH_SERVER_PORT"])
        await site.start()
        logger.info(f"Health server started on port {CONFIG['HEALTH_SERVER_PORT']}")

    async def start(self):
        for symbol in CONFIG["SYMBOLS"]:
            for tf in CONFIG["TIMEFRAMES"]["scalp"] + CONFIG["TIMEFRAMES"]["midterm"]:
                limit = CONFIG["CANDLE_LIMITS"][tf]
                candles = await self.dm.get_klines(symbol, tf, limit)
                if candles: self.dm.candle_buffers[symbol][tf].extend(candles)
        ...
        async def auto_analysis():
            while self.is_running:
                ...
                if self.mentor:
                    full = await self.mentor.full_analysis("SOL-USDT-SWAP")
                    if "اسکالپ لانگ" in full or "اسکالپ شورت" in full:
                        scalp = await self.mentor.scalp_analysis("SOL-USDT-SWAP")
                        combined = f"📊 تحلیل خودکار بازار:\n{full}\n\n🔹 سیگنال دقیق:\n{scalp}" if "⚡ سیگنال اسکالپ" in scalp else f"📊 تحلیل خودکار بازار:\n{full}\n\n(شرایط دقیق برای سیگنال اسکالپ فراهم نشد)"
                    else: combined = f"📊 تحلیل خودکار بازار:\n{full}"
                    try: await self.rep.bot.send_message(chat_id=self.rep.channel, text=combined, parse_mode=ParseMode.MARKDOWN)
                    except: pass
        asyncio.create_task(auto_analysis())

        tasks = [asyncio.create_task(self.engine.run()),
                 asyncio.create_task(self._periodic()),
                 asyncio.create_task(self.tm.run())]
        try: await asyncio.gather(*tasks)
        except asyncio.CancelledError: pass
        finally:
            await self.rep.stop_polling()
            if self.dm.sess: await self.dm.sess.close()
