...
class SharpMoveDetector:
    def __init__(self, dm: MarketDataHub): self.dm = dm; self.m = MathUtils(); self.last_alert = datetime.min.replace(tzinfo=timezone.utc); self.alert_cooldown = 60
    def detect(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        if len(candles_1m) >= 2:
            ...
            if move_pct >= CONFIG["SHARP_MOVE_PCT_THRESHOLD"]:
                ...
                self.last_alert = now
                return {"detected": True, "direction": direction, "price": last_1m.close, "volume_spike": vol_spike}
        return None
    ...
    async def generate(self, symbol: str) -> Optional[Signal]:
        ...
        if self.ml_model.trained:
            ...
            if ml_proba < 0.45:
                ...
                return None
            # Blend AI with rule-based confidence
...
# ---------- MARKET MENTOR ENGINE (OKX adapted, 17 methods) ----------
                    ...
                    if t_closed:
                        if self.signal_gen:
                            ...
                            self.signal_gen.report_result(
                                ...
                                regime=t_closed.regime if hasattr(t_closed, 'regime') else "UNKNOWN",
